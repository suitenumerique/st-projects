...
| `THEME_PREFIX`                             | `''` (recommended)        | White-label by default — leave unset. The only other values, `dsfr`/`anct`, opt into this project's own two built-in government presets (color palette, font, and extra branding elements); not needed for a self-hosted deployment.                                              |
